...
class RecollateOrderableProperties < ActiveRecord::Migration[7.0]
  def up
  ...
  def recreate_generated_columns!
    PROP_TYPES.each do |prop_type|
      ...
      execute prop_type.drop_generate_fn_expression
  ...
  class PropType
    ...
    include Dry::Initializer[undefined: false].define -> do
      ...
      option :collation, Dry::Types["string"].optional, optional: true
    ...
    def add_column_expression
      quoted_collation = " COLLATE #{collation}" if collation.present?

      <<~SQL.strip_heredoc.squish
      ALTER TABLE entity_orderable_properties
        ADD COLUMN #{column_name} #{quoted_pg_type}#{quoted_collation} GENERATED ALWAYS AS (#{generate_expression}) STORED;
      SQL
    end
    ...
    def drop_generate_fn_expression
      <<~SQL.strip_heredoc
      DROP FUNCTION IF EXISTS #{pg_schema}.#{generate_fn_signature}
      SQL
    end

    def generate_fn_signature()
      %[#{generate_fn}(public.schema_property_type, jsonb)]
    end
  ...
  PROP_TYPES = [
    ...
    PropType.new(:string, :citext, collation: "custom_numeric", custom_pg_type: true),
