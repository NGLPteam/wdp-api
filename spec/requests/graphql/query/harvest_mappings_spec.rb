...
RSpec.describe "Query.harvestMappings", type: :request do
  context "when ordering" do
    let(:query) do
      <<~GRAPHQL
      query getHarvestMappingCollection($order: HarvestMappingOrder) {
        harvestMappings(order: $order) {
          edges {
            node {
              id
              slug
    ...
    def create_record(_at:, **attrs)
      Timecop.freeze _at do
        FactoryBot.create(:harvest_mapping, **attrs)
      end
    end
