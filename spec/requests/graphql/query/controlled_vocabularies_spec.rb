...
RSpec.describe "Query.controlledVocabularies", type: :request do
  context "when ordering" do
    let(:query) do
      <<~GRAPHQL
      query getControlledVocabularyCollection($order: ControlledVocabularyOrder) {
        controlledVocabularies(order: $order) {
          edges {
            node {
              id
              slug
    ...
    let(:expected_shape) do
      gql.query do |q|
        q.prop :controlled_vocabularies do |c|
    ...
    def create_record(_at:, **attrs)
      Timecop.freeze _at do
        FactoryBot.create(:controlled_vocabulary, **attrs)
      end
    end
