...
RSpec.describe "Query.controlledVocabulary", type: :request do
  let(:query) do
    <<~GRAPHQL
    query getControlledVocabulary($slug: Slug!) {
      controlledVocabulary(slug: $slug) {
        id
        slug
      }
    }
  ...
  let(:found_shape) do
    gql.query do |q|
      q.prop :controlled_vocabulary do |m|
  ...
  let(:blank_shape) do
    gql.query do |q|
      q[:controlled_vocabulary] = be_blank
    end
  end

  let!(:existing_model) { FactoryBot.create :controlled_vocabulary }
  ...
  shared_examples "a found record" do
    it "finds the ControlledVocabulary" do
  ...
  shared_examples "a not found record" do
    it "does not find the ControlledVocabulary" do
      expect_request! do |req|
