...
RSpec.describe "Query.harvestMessages", type: :request do
  context "when ordering" do
    let(:query) do
      <<~GRAPHQL
      query getHarvestMessageCollection() {
        harvestMessages() {
          edges {
            node {
              id
    ...
    def create_record(_at:, **attrs)
      Timecop.freeze _at do
        FactoryBot.create(:harvest_message, **attrs)
      end
    end
