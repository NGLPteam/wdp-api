...
RSpec.describe "Query.harvestAttempts", type: :request do
  context "when ordering" do
    let(:query) do
      <<~GRAPHQL
      query getHarvestAttemptCollection($order: HarvestAttemptOrder) {
        harvestAttempts(order: $order) {
          edges {
            node {
              id
              slug
    ...
    def create_record(_at:, **attrs)
      Timecop.freeze _at do
        FactoryBot.create(:harvest_attempt, **attrs)
      end
    end
