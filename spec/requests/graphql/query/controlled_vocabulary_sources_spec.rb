...
RSpec.describe "Query.controlledVocabularySources", type: :request do
  context "when ordering" do
    let(:query) do
      <<~GRAPHQL
      query getControlledVocabularySourceCollection($order: ControlledVocabularySourceOrder) {
        controlledVocabularySources(order: $order) {
          edges {
            node {
              id
              slug
    ...
    let(:expected_shape) do
      gql.query do |q|
        q.prop :controlled_vocabulary_sources do |c|
    ...
    def create_record(_at:, **attrs)
      Timecop.freeze _at do
        FactoryBot.create(:controlled_vocabulary_source, **attrs)
      end
    end
