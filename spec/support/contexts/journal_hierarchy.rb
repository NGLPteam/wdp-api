...
module TestHelpers
  class NestedIterator
    ...
    child_iter_klass TestHelpers::NestedIterator
    ...
    include Dry::Initializer[undefined: false].define -> do
      ...
      option :parent, Support::Types.Instance(TestHelpers::NestedIterator).optional, optional: true
    end
...
RSpec.shared_context "journal hierarchy" do
  before_all do
    ...
    @journal_community = TestProf::AnyFixture.register(:journal_community) do
      FactoryBot.create(:community, :with_logo, :with_thumbnail, identifier: "journal-community")
    end

    entities << @journal_community

    @journal = TestProf::AnyFixture.register(:journal) do
      FactoryBot.create :collection, community: @journal_community, schema: "nglp:journal"
    end

    entities << @journal

    jiter = TestHelpers::JournalIterator.new
    ...
    jiter.each do |viter|
      ...
      volume = TestProf::AnyFixture.register(vid) do
        ...
        FactoryBot.create(:collection, schema: "nglp:journal_volume", parent: @journal, title:, identifier:, pending_properties:)
      end
      ...
      viter.each do |iiter|
        ...
        issue = TestProf::AnyFixture.register(iid) do
          ...
          FactoryBot.create(:collection, schema: "nglp:journal_issue", parent: volume, title:, identifier:, pending_properties:)
        end
        ...
        iiter.each do |aiter|
          ...
          article = TestProf::AnyFixture.register(aid) do
            ...
            FactoryBot.create(:item, schema: "nglp:journal_article", collection: issue, title:, identifier:, pending_properties:)
          end
  ...
  TestHelpers::JournalIterator.all_identifiers.each do |identifier|
